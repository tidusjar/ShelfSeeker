/**
 * NZB Search Result Fixture
 */
export interface NzbResultFixture {
  title: string;
  guid: string;
 ...
 */
export const NZB_SEARCH_FIXTURES: Record<string, NzbResultFixture[]> = {
  'dune': [
    {
 ...
 */
export function getNzbResults(query: string): NzbResultFixture[] {
 ...
 */
export function generateNewznabXml(results: NzbResultFixture[], port: number): string {
 ...
 */
export function generateNzbFile(guid: string, filename: string): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
